...
const changeFoodPostion = () => {
    //passing a random 0-30 vlaue as food position
    foodX= Math.floor(Math.random() * 30)+1;
    foodY= Math.floor(Math.random() * 30)+1;
}

const handleGameOver = () => {
    //clearing the timer and reloding the game page on game over 
...
const initGame = () => {
    ...
    if(snakeX === foodX && snakeY === foodY){
        changeFoodPostion();
        snakeBody.push([foodX, foodY]);//pushing the food positon to snake body array
...
//updating the snanke's head position based on the current velocity
    snakeX += velocityX;
    snakeY += velocityY;
